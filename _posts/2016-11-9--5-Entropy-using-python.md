...
- **信息增益**：某个特征A对训练数据集D的信息增益***g(D,A)***，定义为集合D的熵***H(D)***与特征A给定条件下D的条件熵***H(D|A)***之差，即***g(D, A) = H(D) - H(D|A)***。（此处摘自李航《统计学习方法》）即根据观察到的某个特征值，从而减少了对分类或是聚类问题的不确定性（熵）的减少量。它是你减少使用或“做出关于特定变量的决策”的目标类的不确定性的量。也可以理解为你在目标类上获得的信息量，以获得纯类。 参见例如ID3算法。
